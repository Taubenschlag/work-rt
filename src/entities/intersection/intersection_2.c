...
/*   Updated: 2022/10/02 16:37:16 by rokupin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_intersection_list	*co_intrs_cap(t_shape *c, t_ray *r, t_intersection_list *l)
{
	double	t;

	if (!(((t_cone *)c->shape)->closed) || fabs(r->dir->y) < 0.0000001)
		return (l);
	t = (((t_cone *)c->shape)->min - r->origin->y) / r->dir->y;
	if (check_cap_cone_min(r, t, ((t_cone *) c->shape)->min))
		add_intersection(intersect_make_shape(c, t), &l);
	t = (((t_cone *)c->shape)->max - r->origin->y) / r->dir->y;
	if (check_cap_cone_max(r, t, ((t_cone *) c->shape)->max))
		add_intersection(intersect_make_shape(c, t), &l);
	return (l);
}

void	handle_cone_inters(t_ray *ray, t_shape *s,
						   t_intersection_list **ret, double *t_s)
{
	double	a;

	a = ray->origin->y + mins(t_s, 2) * ray->dir->y;
	if (((t_cone *)s->shape)->min < a && a < ((t_cone *)s->shape)->max)
...
t_intersection_list	*intersection_ray_cone(t_shape *s, t_ray *ray)
{
	...
	ret = intersection_list_make(0);
	temp[0] = ray->dir->x * ray->dir->x
		- ray->dir->y * ray->dir->y + ray->dir->z * ray->dir->z;
	temp[1] = 2 * ray->dir->x * ray->origin->x
		- 2 * ray->dir->y * ray->origin->y + 2 * ray->dir->z * ray->origin->z;
	temp[2] = ray->origin->x * ray->origin->x
		- ray->origin->y * ray->origin->y + ray->origin->z * ray->origin->z;
	if (fabs(temp[0]) < 0.000001)
	{
		if (fabs(temp[1]) > 0.0000001)
			add_intersection(intersect_make_shape(
					s, -1 * temp[2] / (2 * temp[1])), &ret);
